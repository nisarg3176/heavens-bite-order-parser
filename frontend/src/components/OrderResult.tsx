  ...
  return (
    <div className={`flex gap-3 ${className}`}>
      ...
      <div className="min-w-0">
        <p className="text-xs uppercase tracking-wide text-bakery-brown/50 mb-0.5">
          {label}
        </p>
        <p className="text-bakery-brown font-medium break-words">
          {value || (
            <span className="text-bakery-brown/40 italic">
              Not specified
            </span>
          )}
        </p>
    ...
    <div className="bg-white rounded-3xl shadow-card border border-white overflow-hidden">
      <div className="px-6 py-5 border-b border-cream-200 flex flex-wrap items-center justify-between gap-3 bg-gradient-to-r from-cream-50 to-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-display text-lg font-bold text-bakery-brown">
              Extracted Order
            </h3>

            {order?.raw_summary && (
              <p className="text-sm text-bakery-brown/60 mt-0.5">
                {order.raw_summary}
              </p>
            )}
          </div>
        ...
        {order?.confidence && (
          <span
            className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide ${
              confidenceColors[order.confidence] ||
      ...
      <div className="px-6 pb-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h4 className="font-semibold text-bakery-brown">
            Ordered Items
          </h4>
        ...
        {items.length === 0 ? (
          <p className="text-bakery-brown/50 italic text-sm">
            No items detected in the conversation.
          </p>
        ) : (
